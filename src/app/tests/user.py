...
def test_update_user(mock_user_service: MagicMock):
    ...
    resp = client.put(
        f"/users/{USER_ID}", json=update.model_dump(exclude_unset=True)
    )
    assert resp.status_code == 200
...
def test_upload_image(
    mock_user_service: MagicMock, mock_image_service: MagicMock
):
    # valid in-memory PNG
    buf = io.BytesIO()
    ...
    mock_image_service.save_image.return_value = (
        "profiles/11111111-1111-1111-1111-111111111111.png"
    )
    expected = {
        **MOCK_USER,
        "picture": mock_image_service.save_image.return_value,
    }
